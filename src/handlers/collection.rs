...
pub async fn sync_put(
    ...
    let details =
        CollectionDetails::from_mapky_collection(collection, user_id, collection_id);
    ...
    graph
        .run(queries::put::create_collection(&details))
        .await?;
    ...
    for osm_url in &collection.items {
        ...
        graph
            .run(queries::put::link_collection_place(&details.id, &osm_canonical))
            .await?;
